...
def real_estate_pipeline():
    ...
    @task
    def fetch_and_store_batch() -> dict:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise RuntimeError(f"Error al consumir la API: {e}")
                import time
                time.sleep(5)
        ...
        with engine2.begin() as conn:
            for i in range(0, len(records), CHUNK_SIZE):
                conn.execute(insert_raw_sql, records[i:i+CHUNK_SIZE])
    ...
    def validate_data_quality(schema_result: dict) -> dict:
        ...
        if not records:
            return {
                **schema_result,
                "quality_valid": False,
                "quality_issues": ["No hay registros válidos en el batch"],
                "null_pct": 0,
                "duplicate_count": 0,
                "num_records": 0,
            }
    ...
    def decide_promotion(compare_result: dict) -> str:
        ...
        if not has_champion or champion_mae is None:
    ...
    raw_result = fetch_and_store_batch()
    ...
    promotion_decision = decide_promotion(compare_task)

    promote_task = promote_model(compare_task)
    reject_task = reject_model(compare_task)

    promotion_decision >> [promote_task, reject_task]
